...
from pipelines.lumina2.managed_model import lumina_models

from task_helpers.progress_bar import ProgressBar

def report(text: str):
    ProgressBar.set_title(f"[LUMINA] {text}")


def generate_lumina_image(model_name: str, input: dict, params: dict):
    ...
    lumina_models.load_models(model_name, mode, controlnet_type, lora_repo_id, lora_scale, transformer2d_model)
    # lumina_models.pipe.scheduler.shift = 3.0 if steps >= 7 else 1.0
    
    if control_image is not None:
        control_args = dict(
            control_image=control_image,
            control_mode=lumina_models.control_mode,
...
def process_workflow_list_model(list_loras: bool):
    return lumina_models.list_models(list_loras)
