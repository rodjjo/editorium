...
from pipelines.lumina2.task_processor import process_workflow_task

from marshmallow import Schema, fields, validate


class LuminaPayloadSchema(Schema):
...
class LuminaTask(WorkflowTask):
    def __init__(self, task_type: str, description: str, is_api: bool = False):
        super().__init__(task_type, description, config_schema=LuminaPayloadSchema, is_api=is_api)

    def process_task(self, input: dict, config: dict) -> dict:
        print("Processing lumina task")
...
def register():
    LuminaTask.register(
        "lumina", 
        "Generate images using a model based on Lumina Image 2.0",
        api_enabled=True
    )
